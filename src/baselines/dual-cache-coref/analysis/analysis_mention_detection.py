...
def get_singleton(input_list):
    ...
    all_men = set(tuple(sublist[0]) for sublist in input_list)
    not_singleton_mention = [sublist for sublist in input_list if len(sublist) > 1]
    not_singleton_mention = flatten(not_singleton_mention)
    not_singleton_mention = set(tuple(sublist) for sublist in not_singleton_mention)
    return all_men - not_singleton_mention

def get_small(input_list):
    ans = input_list[0]
...
def main(gold_clusters, pred_clusters):
    ...
    for gold_cluster, pred_cluster in zip(gold_clusters, pred_clusters):
        ...
        # if is_first:
        #     print("example - ")
        #     print(get_small(in_gold_not_in_pred))
        #     print(get_small(not_in_gold_in_pred))
        #     is_first = False
        ...
        not_in_gold_in_pred_wo_single = set(men for men in not_in_gold_in_pred if men not in singleton_pred_mention)
        ...
        all_pred_mention_single_wo = set(men for men in all_pred_mention if men not in singleton_pred_mention)
