...
class BaseDocumentState:
    def __init__(self, key):
        ...
        self.speakers = []
        self.cluster_str = []
        self.is_multimention = []
        self.segments_is_multimention = []
        self.coref_char_idx = []
        self.words = []
        self.head_words = []
        self.subtoken_length = []
        self.subtokens_words = []
...
def split_into_segment_multimention(document_state):
    count1 = 0
    for part in document_state.segment_subtoken_map:
        is_multimention_seg = document_state.is_multimention[part[0] : part[-1]+1]
        count1 += sum(is_multimention_seg)
        document_state.segments_is_multimention.append(is_multimention_seg)
    return count1
...
def get_tokenizer_new(model_str):
    vocab_size = 16000
    tokenizer_folder = f"../../pretrain_tokenizer/tokenizer/ByteLevelBPETokenizer/{vocab_size}"
    custom_tokenizer = LongformerTokenizerFast.from_pretrained(tokenizer_folder)

    sample_text = "uttaNkasyEva"
    # sample_text = "nfpatistakzakAya"
    # Tokenize the sample text into tokens
