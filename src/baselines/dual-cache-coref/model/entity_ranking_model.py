...
def detach_state(state):
    """Detach tensors in the state dictionary to remove them from the computation graph and print memory size."""
    detached_state = {}
    total_mem_size = 0  # Variable to track total memory used by tensors
    for key, value in state.items():
        if isinstance(value, torch.Tensor):
            if key not in ["mem", 'ent_counter', 'mem_ment_scores'] :
                detached_state[key] = None
                continue

            detached_tensor = value.detach()  # Detach the tensor from the graph
            detached_state[key] = detached_tensor
            
            # Calculate the memory size of the detached tensor
            tensor_mem_size = detached_tensor.element_size() * detached_tensor.nelement()
            total_mem_size += tensor_mem_size
            print(f"Tensor '{key}' size: {tensor_mem_size / (1024 ** 2):.2f} MB")  # Print size in MB
            print(f"shape - {detached_tensor.shape}")
            
        else:
            detached_state[key] = copy.deepcopy(value)  # Deep copy for non-tensor elements

    print(f"Total memory size of detached tensors: {total_mem_size / (1024 ** 2):.2f} MB")  # Print total size in MB
...
class EntityRankingModel(nn.Module):
    ...
    def __init__(self, model_config: DictConfig, train_config: DictConfig, max_ent=None, eval_ent=None, cache_mode=None):
        ...
        self.entity_cluster_states = None
    ...
    def forward_training(self, document: Dict) -> Dict:
        ...
        for idx in range(seg_range[0], seg_range[1]):
            ...
            cur_doc_slice = {
                ...
                "is_multimention": document["is_multimention"][idx]
            }
            ...
            # print(proposer_output_dict["ments"].shape)
        ...
        if self.mem_type == "unbounded":
            ...
            # )
            # coref_new_list = self.memory_net.forward_training(
            #     pred_mentions, mention_emb_list, gt_actions, metadata, ment_scores=ment_scores, cache_mode=self.cache_mode
            # )
            coref_new_list, entity_cluster_states = self.memory_net.forward_training(
                pred_mentions, mention_emb_list, gt_actions, metadata, self.entity_cluster_states, 
                ment_scores=ment_scores, cache_mode=self.cache_mode
            )

            # Detach the entity cluster states to store for the next batch
            # self.entity_cluster_states = detach_state(entity_cluster_states)
            self.entity_cluster_states = None
            del entity_cluster_states
        else:
    ...
    def forward(self, document: Dict, MAX_ENT=600, cache_mode=None) -> Tuple[List, List, List, List]:
        ...
        for idx in range(0, len(document["sentences"])):
            ...
            #         document["subtoken_map"][token_offset] : document["subtoken_map"][token_offset + num_tokens - 1] + 1
            #     ]
