...
class BinaryMultimentionModel(nn.Module):
    def __init__(self, input_size=3092, hidden_size=512):
    ...
    def forward(self, men_vector):
        ...
        logit = torch.sigmoid(self.fc2(x))
        return logit
...
class MentionProposalModule(nn.Module):
    ...
    def _build_model(self, hidden_size: int) -> None:
        ...
        self.multimention_mlp = BinaryMultimentionModel(
            input_size=self.span_emb_size,
            hidden_size=mention_params.mlp_size
        )

        # self.multimention_mlp =  MLP(
        #     input_size=self.span_emb_size,
        #     hidden_size=mention_params.mlp_size,
        #     output_size=1,
        #     num_hidden_layers=mention_params.mlp_depth,
    ...
    def map_is_multimention_to_subtokens(self, is_multimention, subtoken_map, name):
        ...
        for word_idx in subtoken_map:
            try:
                ...
                expanded_is_multimention.append(0)
                print(name)
                print(subtoken_map)
                print(is_multimention)
                print(len(list(set(subtoken_map))))
                print(len(is_multimention), len(subtoken_map))
                print(word_idx, (word_idx - start_idx))
    ...
    def get_token_endpoints(
        ...
        cand_mask: Tensor = constraint1 & constraint2 & constraint3 & constraint4 & constraint5
    ...
    def pred_mentions(
        ...
        # Candidate mention span extraction
        cand_starts, cand_ends, cand_mask = self.get_token_endpoints(
            encoded_doc, document
        )
        # Obtain span embeddings for the candidate mentions
        span_embs = self.get_span_embeddings(encoded_doc, cand_starts, cand_ends)

        multimention_loss = 0

        if self.training:
            # Multi-mention prediction
            is_multimention = document["is_multimention"]
            tensor_multimention = torch.tensor(is_multimention, dtype=torch.float32, device=self.device).view(-1, 1)
            logit = self.multimention_mlp(span_embs)

            # Loss calculation
            criterion = self.loss_fn_multi
            multimention_loss = criterion(logit, tensor_multimention)

            # Backward pass
            # multimention_loss.backward()
        else:
            is_multimention = []
            output = (self.multimention_mlp(span_embs) > 0.5).float() 
            is_multimention = output.view(-1).int().tolist()

        # is_multimention = torch.tensor(is_multimention, dtype=torch.int, device=self.device).view(-1, 1)
        # Ensure alignment between subtoken map and multi-mention predictions
        assert span_embs.shape[0] == len(is_multimention)
        del span_embs  # Span embeddings not required anymore

        # Candidate filtering based on multi-mention predictions
        cand_starts, cand_ends, cand_mask = self.get_candidate_endpoints(
            encoded_doc, document, is_multimention
        )
        # Obtain span embeddings for the filtered candidates
        span_embs = self.get_span_embeddings(encoded_doc, cand_starts, cand_ends)
        # exit()
