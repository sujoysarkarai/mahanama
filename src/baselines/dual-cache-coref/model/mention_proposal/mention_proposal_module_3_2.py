...
class MentionProposalModule(nn.Module):
    ...
    def __init__(self, config, train_config, drop_module=None):
        ...
        # Multi mention loss
        self.multimention_loss_fn = nn.BCELoss()
    ...
    def get_token_endpoints(
        ...
        subtoken_map = torch.tensor(
            document["subtoken_map"] + [-1] * (self.config.mention_params.max_span_width + 1),
            device=self.device,
        )
        
        # Filter out padding (-1) indices
        valid_indices = subtoken_map != -1
        
        # Identify the start of a new word by checking where the value changes
        starts = torch.cat([torch.tensor([True], device=self.device), subtoken_map[1:] != subtoken_map[:-1]]) & valid_indices
        ends = torch.cat([subtoken_map[:-1] != subtoken_map[1:], torch.tensor([True], device=self.device)]) & valid_indices
        
        # Get the indices for candidate starts and ends
        filt_cand_starts = torch.where(starts)[0]
        filt_cand_ends = torch.where(ends)[0]

        return filt_cand_starts, filt_cand_ends


    def map_is_multimention_to_subtokens(self, is_multimention, subtoken_map, name):
        # Initialize a list to hold the expanded multi-mention labels
        expanded_is_multimention = []

        start_idx = subtoken_map[0]
        
        # Iterate through the subtoken_map to assign the correct is_multimention value
        for word_idx in subtoken_map:
            expanded_is_multimention.append(is_multimention[word_idx- start_idx])

        return torch.tensor(expanded_is_multimention, dtype=torch.int, device=self.device)

    
    def get_candidate_endpoints(
        self, encoded_doc: Tensor, document: Dict, is_multimention: List
    ) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        mapped_tensor_multi = self.map_is_multimention_to_subtokens(is_multimention, document["subtoken_map"], document["doc_key"])
        ...
        constraint5 = subtoken_map[cand_starts] == subtoken_map[cand_ends]

        constraint6 = mapped_tensor_multi[cand_starts] == 1

        constraint7 = constraint6 | ( constraint3 & constraint4 )

        cand_mask: Tensor = constraint1 & constraint2 & constraint5 & constraint7
    ...
    def pred_mentions(
        ...
        #Token embedding---------------------------------------------------------------------------------------
        #Candidate token start and ends
        cand_starts, cand_ends = self.get_token_endpoints(
            encoded_doc, document
        )
        # Obtain token embeddings for the candidate mentions
        token_embs = self.get_span_embeddings(encoded_doc, cand_starts, cand_ends)
        
        #Predict if a token contains multi mention / mention + additional words
        # Forward pass through the multi-mention MLP
        multi_mention_prediction = self.multi_mention_mlp(token_embs)
        # multi_mention_prediction will have shape (batch_size, 1), so squeeze it to get (batch_size,)
        multi_mention_prediction = multi_mention_prediction.squeeze(-1)
        # Apply threshold to get binary predictions (0 or 1)
        is_multimention_pred = (multi_mention_prediction >= 0.5).int()


        # Ensure that is_multimention is of the same type and device as multi_mention_prediction
        is_multimention = document["is_multimention"]
        # If is_multimention is a list of 0s and 1s, convert it to a tensor
        tensor_is_multimention = torch.tensor(is_multimention, dtype=torch.float32, device=multi_mention_prediction.device)

        # # Compute the loss
        multimention_loss = self.multimention_loss_fn(multi_mention_prediction, tensor_is_multimention)

        assert token_embs.shape[0] == len(is_multimention)        
        del token_embs
        #-------------------------------------------------------------------------------------------------------
        
        cand_starts, cand_ends, cand_mask = self.get_candidate_endpoints(
            encoded_doc, document, is_multimention_pred
        )
        ...
        if self.training:
            ...
            output_dict["ment_loss"] = mention_loss + multimention_loss
