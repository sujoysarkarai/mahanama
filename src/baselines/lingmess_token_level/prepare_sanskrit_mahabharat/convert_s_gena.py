...
# data_g_map, data_n_map = convert_p.load_data_gn()
# data_map = convert_p.load_data()
data_g_map, data_n_map = dict(), dict()
data_map = dict()
...
def split_jsonlines(file_path, base_dir, test_list, train_ratio=0.8, val_ratio=0.1, test_ratio=0.1):
    ...
    for line in tqdm(lines):
        ...
        file_name = file_name.split("_part_")[0]
    ...
    test_file_with_clusters_heads = os.path.join(base_dir, 'test_file_with_clusters_split_with_clusters_heads.jsonlines')
    ...
    with open(test_file_with_clusters_heads, 'w', encoding='utf-8') as f:
...
def remove_singleton(clusters, clusters_strings, clusters_heads):
    updated_clusters, updated_clusters_strings, updated_clusters_heads = [], [], []
    
    for clu, clu_str, clu_head in zip(clusters, clusters_strings, clusters_heads):
        
        if len(clu) < 2:
            continue
        updated_clusters.append(clu)
        updated_clusters_strings.append(clu_str)
        updated_clusters_heads.append(clu_head)

    return updated_clusters, updated_clusters_strings, updated_clusters_heads

def split_document(doc, max_tokens=1536):
    """
    ...
    clusters_heads = doc['cluster_heads']
    doc_key = doc['doc_key']
...
def conll_to_json(conll_file):
    ...
    for line in lines:
        ...
        if len(parts) >= 3:
            ...
            if len(parts) > 3 and parts[4] != "_": # and "|" not in parts[3]:
...
def jsonlines_construct(folder_path, max_tokens = 1024):
    ...
    for file_path in tqdm(file_paths):
        ...
        with open("example.jsonlines", 'w', encoding='utf-8') as f:
            # json_line = json.loads(line)
    ...
    for doc in tqdm(jsonl_output):
        ...
        if doc_len > max_tokens:
            ...
            for s_docs in split_docs:
                s_docs["clusters"], s_docs["clusters_strings"], s_docs["cluster_heads"] = remove_singleton(s_docs["clusters"], s_docs["clusters_strings"], s_docs["cluster_heads"])
                if len(s_docs["clusters"]) > 0:
                    split_documents_jsonl_output.append(s_docs)
        else:
            doc["clusters"], doc["clusters_strings"], doc["cluster_heads"] = remove_singleton(doc["clusters"], doc["clusters_strings"], doc["cluster_heads"])
            if len(doc["clusters"]) > 0:
                split_documents_jsonl_output.append(doc)

    
    print("long doc count - ", long_doc_count)
    print(f"length of total dataset after split - {len(split_documents_jsonl_output)}")
...
def main(conll_files_path, jsonl_file_path, test_csv_file_path):
    ...
    output_jsonl_file = os.path.join(output_directory, 'gena_file_with_clusters.jsonlines')
...
if __name__ == "__main__":
    main('../../final_data/conll_format/gena_v3/', './', '../../final_data/conll_format/list_of_possible_test_subchapters.csv')
