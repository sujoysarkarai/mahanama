...
from lingmess_token_level.util_original import extract_clusters, extract_mentions_to_clusters, mask_tensor, get_pronoun_id, get_category_id
...
class LingMessCoref(BertPreTrainedModel):
    ...
    def _prune_topk_mentions(self, mention_logits, attention_mask):
        ...
        # print(topk_mention_start_ids)
        # print(topk_mention_end_ids)
    ...
    def _get_categories_labels(self, tokens, subtoken_map, new_token_map, span_starts, span_ends):
        ...
        for b, (starts, ends) in enumerate(zip(span_starts.cpu().tolist(), span_ends.cpu().tolist())):
            ...
            for start, end in zip(starts, ends):
                ...
                span = [tokens[b][idx] for idx in token_indices if idx is not None]
                # print(span)
                
                pronoun_id = get_pronoun_id(span)
                doc_spans.append((span, pronoun_id))
    ...
    def _calc_mention_logits_length_one(self, start_mention_reps, end_mention_reps, subtoken_map):
        """
        Compute mention logits but only for mentions of length one (single-token mentions).
        
        :param start_mention_reps: Tensor of shape [batch_size, seq_length, hidden_dim]
        :param end_mention_reps: Tensor of shape [batch_size, seq_length, hidden_dim]
        :param subtoken_map: Tensor of shape [batch_size, seq_length], mapping subtokens to token indices.
        :return: mention_logits only for single-token mentions.
        """
        batch_size, seq_length, _ = start_mention_reps.shape
        
        # subtoken_map = [[0 if v is None else v for v in row] for row in subtoken_map]
        # subtoken_map = torch.tensor(subtoken_map, dtype=torch.long, device=start_mention_reps.device)

        # Convert None values to 0 and make each row a tensor
        subtoken_map = [torch.tensor([0 if v is None else v for v in row], dtype=torch.long) for row in subtoken_map]

        # # Pad all sequences to the maximum sequence length in the batch
        subtoken_map = pad_sequence(subtoken_map, batch_first=True, padding_value=0).to(start_mention_reps.device)

        # Compute standard mention logits
        start_mention_logits = self.mention_start_classifier(start_mention_reps).squeeze(-1)  # [batch_size, seq_length]
        end_mention_logits = self.mention_end_classifier(end_mention_reps).squeeze(-1)        # [batch_size, seq_length]

        temp = self.mention_s2e_classifier(start_mention_reps)                                # [batch_size, seq_length, hidden_dim]
        ...
        # Compute valid single-token mentions (start and end must map to the same token)
        start_tokens = subtoken_map.unsqueeze(1).expand(-1, seq_length, -1)  # [batch_size, seq_length, seq_length]
        end_tokens = subtoken_map.unsqueeze(2).expand(-1, -1, seq_length)    # [batch_size, seq_length, seq_length]
        
        single_token_mask = (start_tokens == end_tokens).float()  # Convert boolean to float

        # Apply mask using multiplication (to ensure dtype consistency)
        valid_mentions = mention_mask * single_token_mask

        # Mask out invalid mentions
        mention_logits = mask_tensor(mention_logits, valid_mentions)  # [batch_size, seq_length, seq_length]
        # print(mention_logits.tolist())
        print(mention_logits.shape)
        # exit()

        return mention_logits

    def _calc_mention_logits_length_one_boundary(self, start_mention_reps, end_mention_reps, subtoken_map):
        """
        Compute mention logits for single-token mentions, ensuring subtoken length covers the whole token.
        
        :param start_mention_reps: Tensor of shape [batch_size, seq_length, hidden_dim]
        :param end_mention_reps: Tensor of shape [batch_size, seq_length, hidden_dim]
        :param subtoken_map: Tensor of shape [batch_size, seq_length], mapping subtokens to token indices.
        :return: mention_logits only for valid single-token mentions covering entire tokens.
        """
        batch_size, seq_length, _ = start_mention_reps.shape

        # Convert None values to 0 and make each row a tensor
        subtoken_map = [torch.tensor([-1 if v is None else v for v in row], dtype=torch.long) for row in subtoken_map]
        ...
        single_token_mask = (start_tokens == end_tokens)

        # Additional constraints to ensure entire token is covered
        # Check that the word corresponding to the previous subword is not the same at span start
        start_valid = torch.cat(
            [torch.zeros(batch_size, 1, device=start_mention_reps.device, dtype=torch.bool),  # First subtoken is always valid
            subtoken_map[:, 1:] != subtoken_map[:, :-1]], dim=1)

        # Check that the word corresponding to the next subword is not the same at span end
        end_valid = torch.cat(
            [subtoken_map[:, :-1] != subtoken_map[:, 1:],  # Compare with the next subtoken
            torch.zeros(batch_size, 1, device=start_mention_reps.device, dtype=torch.bool)], dim=1)  # Last subtoken is always valid

        valid_idx = start_valid[:, :, None] & end_valid[:, None, :]

        idx = torch.arange(valid_idx.shape[1], device=valid_idx.device)
        valid_idx[:, idx, idx] = False

        # boundary_mask = start_valid.unsqueeze(1) * end_valid.unsqueeze(2)  # Combine start and end constraints

        c_mask = single_token_mask & valid_idx

        valid_mentions = mention_mask * c_mask.float()
        mention_logits = mask_tensor(mention_logits, valid_mentions)
    ...
    def forward(self, batch, gold_clusters=None, return_all_outputs=False):
        ...
        # mention_logits = self._calc_mention_logits_length_one(start_mention_reps, end_mention_reps, subtoken_map)
        mention_logits = self._calc_mention_logits_length_one_boundary(start_mention_reps, end_mention_reps, subtoken_map)

        # prune mentions
        mention_start_ids, mention_end_ids, span_mask, topk_mention_logits = self._prune_topk_mentions(mention_logits, attention_mask)
        # mention_start_ids, mention_end_ids, span_mask, topk_mention_logits = self._prune_topk_mentions_length_one(mention_logits, attention_mask, subtoken_map)
