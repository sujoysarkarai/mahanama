...
from refined.offline_data_generation.preprocessing_utils import download_url_with_progress_bar, download_first_n_lines
...
def download_dumps_in_parallel():
    ...
    """
    print('Downloading Wikidata and Wikipedia dumps.')
...
def download_dumps():
    ...
    download_first_n_lines(WIKIDATA_DUMP_URL, os.path.join(OUTPUT_PATH, WIKIDATA_DUMP_FILE))
    download_first_n_lines(WIKIPEDIA_ARTICLES_URL, os.path.join(OUTPUT_PATH, WIKIPEDIA_ARTICLES_FILE))
...
def main():
    ...
    print('Step 3) Processing Wikipedia redirects dump.')
    ...
    print('Step 4) Extract text from Wikipedia dump.')
    ...
    print('Step 5) Building PEM lookup.')
    ...
    print('Step 6) Building entity index from PEM.')
    ...
    print('Step 7) Building descriptions tensor.')
    ...
    print('Step 10) Creating class labels lookup')
    if not os.path.exists(os.path.join(OUTPUT_PATH, 'class_to_label.json')):
        build_class_labels(OUTPUT_PATH)

    print('(Step 11) Training MD model for ontonotes numeric/date spans (date, cardinal, percent etc.)')
    ...
    if len([x[0] for x in list(os.walk(OUTPUT_PATH)) if model_dir_prefix in x[0]]) == 0:
        ...
        print('Model already trained so skipping')

    print('Step 12) Relabelling CONLL dataset using numeric/date MD model')
    if not os.path.exists(os.path.join(OUTPUT_PATH, "datasets", "conll_train_plus_dates.txt")):
        ...
        print('Already relabelled CONLL dataset so skipping')

    print('Step 13) Train MD model on augmented MD datasets')
    ...
    if len([x[0] for x in list(os.walk(OUTPUT_PATH)) if model_dir_prefix in x[0]]) == 0:
        ...
        print("Found an MD model already trained on augmented MD datasets, so skipping")

    print('Step 14) Running MD model over Wikipedia.')
    ...
    print('Step 15) Building LMDB dictionaries and storing files in the expected file structures.')
    build_lmdb_dicts(preprocess_all_data_dir=OUTPUT_PATH, keep_all_entities=keep_all_entities)

    print("The preprocess_all.py script is done. You can now use the newly generated/updated data files "
             "for your trained model or train a model from scratch on the newly generated Wikipedia dataset.")
    print(f"The data_dir is the relative path: {OUTPUT_PATH}/organised_data_dir.")
    print(f"You can train a model with the new data using `train.py --download_files n "
             f"--data_dir {OUTPUT_PATH}/organised_data_dir` . Ensure --download_files n to avoid overwriting.")
    print(f"You can use an existing model with the updated data files (e.g. includes recently added entities) "
