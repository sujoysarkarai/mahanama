...
def human_qcode(qcode_to_class_type_names, is_test: bool = False):
    ...
    for c, n in class_code_to_class_name.items():
        if c in human_class_code:
    ...
    with open(class_type_file, "r") as file:
        for line in file:
            ...
            if len(parts) == 2:  # Ensure the line has exactly two parts
                key, value = parts
                class_code_to_class_name[key] = value
                cqcode = "Q555" + str(i)
                class_name_to_class_qcode[value] = cqcode
                i += 1
...
def get_class_type(ner_type_file):
    ...
    with open(ner_type_file, mode='r') as file:
        reader = csv.DictReader(file)  # Automatically uses the first row as headers
        for row in reader:
            cluster_id = row['cluster_head_id']
            if cluster_id in cluster_id_to_qcode:
                try:
                    class_name = class_code_to_class_name[row['entity_tag']]
                    qcode_to_class_type_names[cluster_id_to_qcode[cluster_id]].add(class_name)
                except:
                    pass
            # exit()
...
# Example usage
ner_type_file = '../../final_data/conll_format/cluster_data_for_manual_classification_of_ner_labels.xlsx - Sheet1.csv'  
class_type_file = '../../final_data/conll_format/classes.txt'  # Replace with your titles file path
