...
def download_first_n_lines(url: str, output_path: str, n: int = 20000, redownload: bool = False):
        ...
        with urllib.request.urlopen(url) as response, open(temp_file, 'wb') as out_file:
            with DownloadProgressBar(unit='B', unit_scale=True, miniters=1, desc=url.split('/')[-1]) as t:
                ...
                for line in response:
                    out_file.write(line)
                    lines_downloaded += 1
                    if lines_downloaded >= n:
                        break
                    t.update(len(line))
        
        os.rename(temp_file, output_path)
        print(f"Downloaded the first {n} lines to {output_path}.")
    else:
