...
class RefinedModel(nn.Module):
    ...
    def __init__(
        ...
        # self.transformer_config  = deepcopy(preprocessor.transformer_config).config  # for robarta remove this

        # sub-modules
        self.mention_detection: nn.Module = MentionDetection(
            num_labels=len(config.ner_tag_to_ix),
            dropout=config.md_layer_dropout,
            hidden_size=self.transformer_config.hidden_size # for robarta remove this
        )
    ...
    def forward(
        ...
        # print(batch.token_id_values.shape)
        # print(batch.attention_mask_values.shape)
        # print(batch.token_type_values.shape)
        ...
        if batch.token_acc_sum_values is None:
            ...
            # with open("batch_ex_argu.txt","w") as f:
            #     f.write(F"cand_ids, candidate_pem_values, candidate_entity_targets, cand_desc, cand_desc_emb - ")
            #     f.write(F"{str(expanded_args)}")
            # exit()
            ...
            # with open("batch_cand_class.txt","w") as f:
            #     f.write(F"{str(batch.candidate_class_values)}")
                # f.write(F"{candidate_classes}")
            # exit()
            # cand_desc_emb = None
            ...
            other_spans = {}
            # with open("batch_entity_span.txt","w") as f:
            #     f.write(F"{str(entity_spans)}")
            # exit()

        class_targets = self._expand_class_targets(
            batch.class_target_values, index_tensor=batch.entity_index_mask_values
        )
        # with open("batch_class_target.txt","w") as f:
        #     f.write(F"{str(class_targets)}")
        # exit()
        # print("here1")
        ...
        # with open("batch_mem_emb.txt","w") as f:
        #     f.write(F"{str(mention_embeddings)}")
        # exit()
        ...
        # with open("batch_mem_desc.txt","w") as f:
        #     f.write(F"{str(description_loss)}")
        #     f.write(F"{str(candidate_description_scores)}")
        # exit()
        # print("here3")
        ...
        # et_loss, et_activations = None, None
        # with open("batch_et.txt","w") as f:
        #     f.write(F"{str(et_loss)}")
        #     f.write(F"{str(et_activations)}")
        # exit()
        # print("here4")
        ...
        # with open("batch_ed.txt","w") as f:
        #     f.write(F"{str(ed_loss)}")
        #     f.write(F"{str(ed_activations)}")
        # exit()
        # print("here5")
        return ModelReturn(
            md_loss,
