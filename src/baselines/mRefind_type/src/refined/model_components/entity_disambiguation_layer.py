...
class EntityDisambiguation(nn.Module):
    ...
    def forward(
        ...
        # candidate_diff_classes = candidate_classes
        ...
        if self.ignore_types:
            # [18, 10, 1861]
            candidate_delta_classes = torch.zeros_like(candidate_classes , device=current_device) # 81 5 1369
            del candidate_classes
            # candidate_delta_classes = torch.zeros([138, 138], dtype = torch.long)
            candidate_dist = torch.zeros_like(candidate_dist , device=current_device)  # 81 5  1
        else:
