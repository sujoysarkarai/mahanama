    ...
    for epoch_num in trange(fine_tuning_args.epochs):
        ...
        for step, batch in tqdm(enumerate(training_dataloader), total=len(training_dataloader)):
            ...
            with autocast():
                ...
                # print("loss-")
                # print(output.ed_loss, output.et_loss, output.description_loss)
                if output.ed_loss is None:
                    loss = 0
                    # exit()
                else:
            ...
            if step % 100 == 99:
                LOG.info(f"TLoss: {total_loss / step}")
            ...
            if (step + 1) % checkpoint_every_n_steps == 0:
                print("check point - ", checkpoint_every_n_steps," and steps - ", step)
        ...
        print("check point end - ", checkpoint_every_n_steps," and steps - total")
        best_f1 = run_checkpoint_eval_and_save(best_f1, evaluation_dataset_name_to_docs, fine_tuning_args,
                                               refined, optimizer=optimizer, scaler=scaler,
                                               scheduler=scheduler)
        # exit()
