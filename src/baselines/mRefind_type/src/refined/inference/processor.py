...
class Refined(object):
    ...
    def process_tensors(self, batch: BatchedElementsTns, ner_threshold: float = 0.5,
        ...
        device = output.ed_activations.device
        ...
        span_indices, pred_class_indices = torch.nonzero(
            ...
            # output.candidate_description_scores > ner_threshold, as_tuple=True
        )
        for span_idx, pred_class_idx, conf in zip(
                ...
                round_list(
                    ...
                    # output.candidate_description_scores[(span_indices, pred_class_indices)].cpu().numpy().tolist(), 4
                ),
        ):
            if pred_class_idx == 0:
