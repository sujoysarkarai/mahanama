...
from refined.evaluation.analysis_entity_error import analysis
...
def process_annotated_document(
    ...
    for span in doc.spans:
        ...
        gold_dict[span.gold_entity.wikidata_entity_id].append([span.start, span.start + len(span.text)])
        if span.gold_entity.wikidata_entity_id in {qcode for qcode, _ in span.candidate_entities}:
            gold_entity_in_cands += 1
            # if el is False:
            #     print("gold entity - ", doc.doc_id, span.text, span.start, span.gold_entity.wikidata_entity_id)
        else:
            if el is False:
                print("new surface form - ", doc.doc_id, span.text, span.start, span.gold_entity.wikidata_entity_id)
    ...
    for span in predicted_spans:
        ...
        pred_dict[qcode].append([span.start, span.start + len(span.text)])
    ...
    # print(len(gold_dict.items()), len(pred_dict.values()))
    metrics = Metrics(
        el=el,
        ...
        num_docs=1,
        example_errors=[{'doc_title': doc.doc_id, 'fp_errors': fp_errors, 'fn_errors': fn_errors}],
        example_errors_md=[{'doc_title': doc.doc_id, 'fp_errors_md': fp_errors_md, 'fn_errors_md': fn_errors_md}]
    )
    return metrics, list(gold_dict.values()), list(pred_dict.values())
...
def evaluate_on_docs(
    ...
    for doc_idx, doc in tqdm(
        ...
        doc_metrics, g_cluster, p_cluster = process_annotated_document(
            refined=refined,
            doc=doc,
        ...
        gold_clusters.append(g_cluster)
        pred_clusters.append(p_cluster)
        overall_metrics += doc_metrics
        if sample_size is not None and doc_idx > sample_size:
            break
    # print(len(gold_clusters))
    # print(gold_clusters[2])
    # print(pred_clusters[2])
    analysis(gold_clusters=gold_clusters, pred_clusters=pred_clusters)
