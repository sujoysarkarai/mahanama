...
def predict(gold_clusters, pred_clusters, gold_clusters_str = [], gold_clusters_heads = []):
    ...
    total_gold_mention = 0
    total_pred_mention  = 0
    cc = 0
    ...
    for gold_cluster, pred_cluster in zip(gold_clusters[:], pred_clusters[:]):
        ...
        for i in range(len(gold_cluster)):
            ...
            for x in count.keys():
                ...
                if corr_gold_num == count[x]:
                    if corr_pred_all_clu_num == corr_gold_num:
                        is_correct = True
                        clu_map = [clu, [pred_cluster[x]], "correct", "", ""]
                        break
                    elif corr_pred_all_clu_num > corr_gold_num:
                        is_conflated = True
                        clu_map = [clu, [pred_cluster[x]], "conflated", "", ""]
                        break
            ...
            if is_correct :
                ...
                clu_map = [clu, clu_divided_list, "divided", "", ""]
                divided_cluster += 1
            else: 
                clu_map = [clu, [], "missing", "", ""]
                missing_cluster += 1
    ...
    print("gold_mention_count = ", total_gold_mention)
    print("pred_mention_count = ", total_pred_mention)
...
def analysis(gold_clusters, pred_clusters, gold_clusters_str = [], gold_clusters_heads = []):
...
if __name__ == "__main__":
    ...
    analysis(gold_clusters, pred_clusters)
