...
class ClassHandler:
    ...
    def _explore_class_tree(
        ...
        #     return frozenset(explored_classes)  # always this 
    ...
    def get_classes_idx_for_qcode_batch(
        ...
        # print(result.shape, len(qcodes))

        # result = torch.zeros([len(qcodes), 138], dtype = torch.long)
        # print(result.shape, len(qcodes))
        # with open("get_classes_idx_for_qcode_batch.txt","w") as f:
        #     f.write(F"{str(qcodes)}")
        #     lis = [self.qcode_to_idx[qcode] if qcode in self.qcode_to_idx else 0 for qcode in qcodes]
        #     for i in lis:
        #         f.write(f"{str(i)} - {str(self.qcode_idx_to_class_idx[i])}\n")
        #     f.write(F"{str(self.qcode_idx_to_class_idx[[self.qcode_to_idx[qcode] if qcode in self.qcode_to_idx else 0 for qcode in qcodes]])}")
        # exit()
