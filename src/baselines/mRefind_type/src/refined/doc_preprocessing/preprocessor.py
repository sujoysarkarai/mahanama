...
class PreprocessorInferenceOnly(Preprocessor):
    ...
    def split_sentences(self, text: str):
        ...
        @return: list of tuple of (start, end) offsets
        """
        pattern = r"\.\."  # Matches sentence endings defined by ".."
        start = 0

        for match in re.finditer(pattern, text):
            end = match.end()  # Get the end offset of the match
            yield (start, end)
            if end + 2 <= len(text) and text[end:end+2] == "\n\n":
                start = end + 2  # 2-character gap (newlines)
            else:
                start = end + 1  # 1-character gap (normal)
